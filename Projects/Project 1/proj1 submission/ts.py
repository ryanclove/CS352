...
parser = argparse.ArgumentParser(description="""This is a very basic top level DNS server program""")
...
with csockid:
    while True:
        ...
        for line in open("PROJI-DNSTS.txt", 'r'):
            parse_line = line.split()
            if parse_line[0].lower() == query.lower():
                csockid.sendall(line.encode('utf-8'))
                found = True
                break
        # query not in table, must return ERROR
        if not found:
            response = query + " - Error:HOSTNOTFOUND"
            csockid.sendall(response.encode('utf-8'))
        found = False
